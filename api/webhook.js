export default async function handler(req, res) {
  ...
  try {
    ...
    const nombre = order.shipping_address?.name || order.billing_address?.name || 'Sin nombre';
    const direccion = formatDireccion(order.shipping_address || order.billing_address);
    const telefono = order.shipping_address?.phone || order.billing_address?.phone || order.phone || 'Sin telefono';
    const email = order.email || 'Sin email';
    const orderNumber = order.order_number || order.name || 'S/N';
